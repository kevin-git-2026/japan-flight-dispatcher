...
def _arrival_tail_keys(graph, arr, endpoint_key):
    """据 VATJPN「到着」尾段给出本次 A* 落点之后应【直飞(DCT)补全】的尾段 keys（按 `_arr_dct_anchors` 的可达锚点查表——
    落点=某进场门(RJFM KUE)或重锚点(RJFF FUGEN) 时才有）；逐点解析坐标，遇解析不到 / 明显背离本场(超回退阈) 即截断。返回 [key,…]（可空）。"""
...
def _prefer_corridor_arrival(dep, arr, graph, best, exit_dct, run):
    ...
    tk = _arrival_tail_keys(graph, arr, best["fixes"][-1])     # 当前航路(含到着尾段)的实际末点
...
def _direct_route(dep, arr, graph, dat_path):
    ...
    进场走廊奖励：A* 纯最短的落点换成真实 AIP 最常用的方向合规进场门（容差内）。无解 → None。"""
    ...
    if best:
        ...
        if best["suspect"] or best["dist_nm"] > gc * _GATE_GIVEUP_RATIO:    # 离场门导致坏航路 → 弃门重算
            ...
            if alt and ((best["suspect"] and not alt["suspect"]) or alt["dist_nm"] < best["dist_nm"]):
                best = alt
    best = _prefer_corridor_arrival(dep, arr, graph, best, exit_dct, _run)   # 进场走廊奖励（常用门 ≤1.15×最短则优先）
