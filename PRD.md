...
| 当前版本 | v1.2.0 |
| 交付形态 | 单文件 Python CLI / PyInstaller 打包的 `flight_dispatcher.exe` |
...
### 2.4 启动时的 Volanta 同步询问(F11 相关)

是否在启动时询问,由**同步偏好**(`volanta_data.json` 的 `preference` 字段)决定:

- **已开启自动同步**(此前选过 Y,文件内容为 `auto`)→ 启动**静默自动同步**(用浏览器里的登录会话拉取),**不再询问、不开窗**;登录过期则沿用已保存的已飞数据。
- **未开启**(从未选过 / 选过 N 或回车)→ **每次启动都询问一次**:

| 提示项 | 输入 | 行为 |
| --- | --- | --- |
| 是否同步 Volanta 已飞数据 | `Y` | 先用浏览器里已有的 Volanta 登录会话直接拉取(无需开窗);取不到有效令牌才打开 Edge(回退默认浏览器)跳转 `fly.volanta.app/flights` 让用户**登录**,轮询直到拉到 `/api/v1/Flights` 数据(无需滚动,最长 3 分钟)。**并把偏好记为 `auto`,以后启动自动同步、不再询问** |
| 是否同步 Volanta 已飞数据 | 回车 / N | 本次跳过同步,但仍读取现有本地数据(手动导出的 json / CSV / 累积库);**保持「每次询问」,下次启动再问** |

> 行为约定:
> - **偏好持久化** —— 选 Y 即把 `volanta_data.json` 的 `preference` 记为 `auto`,此后自动同步;想恢复每次询问,把它改回 `ask`(或删除 `volanta_data.json`——注意删除会一并清空已飞累积库)。
> - **不锁死** —— 未选 Y 时每次启动都会再问,后来才开始用 Volanta 的用户不会被永久跳过。
> - **隐私** —— 仅在「已开启自动」或「本次选了 Y」时,程序才会去读浏览器 Local Storage 找登录令牌;未开启且未选 Y 时绝不碰浏览器。
...
启动
  │
  ├─[一次性] 定位导航数据 (find_xp_data_files：自带 NavData 优先 → 同级 → XP)
  │            └─ AIRAC 自检 (check_airac_currency：cycle_info.txt 过期则提示更新)
  │
  ├─[一次性] 多源地景扫描 (scan_installed_sceneries：XP Custom Scenery + MSFS Community
  │            → dict{ICAO: 来源}，installed_scenery.json 指纹缓存，目录未变秒开)
  ├─[一次性] 加载 AIP 航路数据 (load_aip_routes_from_csv,网络优先 → 本地缓存)
  ├─[一次性,可选] Volanta 已飞航线获取(由 volanta_data.json 的偏好决定)
  │            ├─ 已开启自动(auto):静默用登录会话(localStorage 的 Orbx token)调
  │            │   /api/v1/Flights → 解析起降对并入 volanta_data.json(零操作、不再询问)
  │            ├─ 未开启:每次启动询问;选 Y → 拉取(必要时开浏览器登录)+ 记住自动;N/回车 → 跳过
  │            └─ load_volanta_flown_routes:读累积库(+可选 json/CSV 导入)→ 已飞有向航线
  │
  └─[循环] 交互式规划
            ├─ 读取本轮输入(机场 / 航司 / 机型 / 时间 / 跑道 / 航程 / 严格模式)
            ├─ 重新加载机场列表 (load_airports_from_navigraph,因跑道阈值每轮可变)
            ├─ 选择航线
            │    ├─ 双固定:直接计算大圆距离 + 查 AIP 航路
            │    └─ 含随机:get_random_route 枚举候选 + 按已飞次数加权抽取
            │       (优先未飞航线;按军用端数量分层优先民用机场)
            ├─ 渲染规划单 (print_flight_info 闭包)
            └─ 抓取现实排班 (fetch_real_flights_with_filter) 或降级模拟呼号(按航线挑航司)
```

### 3.2 代码组织

v1.2.0 起,原本约 1100 行的单文件 `flight_dispatcher.py` 已**按功能拆分为 `dispatcher/` 子包**(每个模块一个职责),便于维护;根目录的 `flight_dispatcher.py` 退化为**薄壳入口**(仅 `from dispatcher.app import main; main()`),保持运行/打包命令不变。仍为纯标准库实现。
...
| `dispatcher/navdata.py` | `find_navdata_file`、`check_airac_currency`、`locate_xp_root`、`find_xp_data_files` | 定位导航数据(自带 NavData 优先)、AIRAC 自检、定位 XP 根 |
...
| `dispatcher/volanta.py` | `try_fetch_volanta_json_via_session`、`load_volanta_flown_routes`、`sync_volanta_via_browser`、`prompt_sync_volanta`、`volanta_synced_today` | 用登录会话(Orbx token)拉 `/api/v1/Flights` 自动生成 json → 解析 Volanta 已飞有向航线 |
...
| `dispatcher/routing.py` | `calculate_distance_nm`、`find_aip_route`、`get_random_route` | 大圆距离、AIP 匹配、航线抽取(软优先未飞) |
| `dispatcher/app.py` | `main`、`print_flight_info`(闭包) | 启动初始化 + 无限规划循环 |
...
### 4.1 导航数据定位 (`find_xp_data_files` / `find_navdata_file`)

**导航数据(`earth_aptmeta.dat`)定位**(v1.2.0 续起以**自带 NavData** 为主,F12):

0. **程序自带 `NavData` 文件夹(主方案)**:读 `<程序目录>\NavData\earth_aptmeta.dat`,**彻底解耦 XP**,只飞 MSFS、没装 XP 的用户也可用。
1. **exe 同级目录** 的 `earth_aptmeta.dat`(`.ini` 可选)。
2. **X-Plane 安装**:`installed_scenery.json` 记忆的 `xp_root` / 全盘扫描(经 `locate_xp_root`)。
3. **手动输入** XP 根目录。
...
> `get_real_run_path()` 决定所有同级文件(`xp_path_config.txt`、`routes_cache.csv`)的锚点:冻结(exe)模式取 exe 所在目录,源码模式取脚本所在目录。
>
> 兼容两种导航数据存放层级:`Custom Data/earth_aptmeta.dat` 与 `Custom Data/earth_nav_data/earth_aptmeta.dat`。
...
- **浏览器同步(登录兜底)**:在「未开启自动、用户本次选 Y」且本机已有登录会话取不到有效令牌时触发;打开 `fly.volanta.app/flights` 让用户**登录**,随后**轮询 `try_fetch_volanta_json_via_session` 直到新令牌出现、API 拉取成功**(无需滚动,最长 3 分钟兜底)。同步偏好持久化在 `volanta_data.json` 的 `preference`(`auto`)(详见 2.4)。
...
| 维度 | 要求 |
| --- | --- |
| 部署 | 单文件 exe,免安装;`pyinstaller --onefile flight_dispatcher.py` 构建 |
...
| 健壮性 | 主循环捕获异常并打印 `❌ 发生错误`,单轮失败不影响后续规划 |
...
[启动初始化 — 仅一次]
  print 欢迎横幅
  find_xp_data_files()            # 定位导航数据(自带 NavData 优先)
  ...
  load_aip_routes_from_csv()      # 加载 AIP(网络优先)
  # Volanta:按 volanta_data.json 偏好——已开启(auto)则静默用登录会话拉 API；
  #          未开启则每次启动询问(选 Y 拉取并记住自动 / N 回车跳过)
...
[规划循环 — 无限]
  while True:
    读取本轮输入
    解析跑道长度 / 航程范围
    load_airports_from_navigraph()         # 按本轮跑道阈值重载机场
    if 机场列表为空: continue
    ...
    if 出发与目的均固定:
        直接计算距离 + 查 AIP 航路
        if 严格模式且无 AIP: 报错
        else: print_flight_info()
    else:
        get_random_route(flown_routes)     # 枚举候选 + 加权抽取(1/(count+1)²)；按军用端数量分层优先民用
        print_flight_info()                # 触发现实排班抓取(无排班→按航线挑航司生成模拟呼号)；
                                           # 已飞航线标注 [⚠️Volanta:已飞过 N 次]
