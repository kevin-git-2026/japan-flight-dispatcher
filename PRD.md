...
| 当前版本 | **v2.0.1**（F27 严格基于现实航班：FlightAware 逐候选核实 + F28 按地理地域限定：经纬度 9 地域分类）；v2.0.0_alpha1（F26 羽田实测运用状况：ntrack 实测优先于规则推断）；v2.0.0（F25 UI 迁移 tkinter → Flet：Flutter 渲染，业务逻辑零改动）；v1.6.0（运行规则应用引擎 F24）；v1.5.0（天气网格回退 F22 + 运行规则编辑器 F23）；v1.4.x（本地航路生成 + SimBrief + 机型库 + 交互地图） |
...
| F26 | 羽田实测运用状况（✅ v2.0.0_alpha1） | 抓**国土交通省 ntrack**「羽田空港飛行コース」全公开的**实测运用状况**（当前在用的 进近方式 + 落地跑道 + 离场跑道，每 30 分钟一条）→ **进离场预选的首选依据**，优先于按「时段+风+天气」**推断**的规则引擎（F24）。多条跑道在用时，比对 `operation.json` 找出最吻合的那条规则，据此为用户默认算出跑道 + SID/STAR。**仅羽田**；其余机场仍走规则引擎。取不到 → 静默回退。详见 §8.2 |
...
| F27 | 严格基于现实航班（✅ v2.0.1） | 随机抽线新增开关「严格基于现实航班」：抽出的航线须**现实中真有航班在飞**——`controller` 逐候选调 **FlightAware** 核实（填了执飞航司→该航司必须在飞；留空→任一直飞航班），命中即返回、上限 12 条、日志逐条报进度；全灭 best-effort 回退并诚实告警。仅作用于**随机端**；固定双端则仅核实+提示不改端。用 **权威实时源** 而非会过期的手录网络。详见 §8.2 |
| F28 | 按地理地域限定（✅ v2.0.1） | 随机抽线新增「出发地域 / 目的地域」下拉：把起飞、落地机场各限定在日本 **9 大地域**（北海道/东北/关东/中部/近畿/中国/四国/九州/琉球）之一。新增 `dispatcher/regions.py` 按**机场经纬度 bounding box** 分类（非 ICAO 前缀——前缀分不开中国/四国且本身有错）：每地域一组盒、九州按关门海峡纬度线分东西两盒、按优先级顺序判定；实测 129 机场 0 未归类、61 边界机场 0 误分。仅作用于**随机端**。详见 §8.2 |
...
| `dispatcher/routing.py` (v2.0.1 扩展) | `calculate_distance_nm`、`find_aip_route`、`get_random_route`(含 `require_both_scenery` 过滤;**v2.0.1** + `dep_region`/`arr_region` 地域预过滤 + `accept_fn`/`on_verify`/`on_giveup` 现实航班核验循环) | 大圆距离、AIP 匹配、航线抽取(软优先未飞 + 可选仅地景机场 + 可选地域限定 + 可选 FlightAware 现实核验)。网络 I/O 不进本模块——`accept_fn` 由 `controller` 注入 |
| **`dispatcher/regions.py`** (F28,✅ v2.0.1) | `REGIONS`、`region_of(lat, lon)`、`_REGION_BOXES` | 日本 9 大地理地域的**经纬度 bounding box** 分类。每地域一组盒、九州按关门海峡纬度线分东西两盒、按优先级顺序判定;实测 129 机场 0 未归类、61 边界机场 0 误分。纯 stdlib、零 GUI、零 import 依赖 |
...
| **`dispatcher/ntrack.py`** (F26,✅ v2.0.0_alpha1) | `fetch_latest`、`parse_config`、`match_iaps`、`supports` | 国土交通省 ntrack 的**实测运用状况**（羽田）。一次 GET，表格内联在首页 HTML（`<table id='atistable'>`），无 ajax / 无 JS / 无认证；30 分钟缓存＝其更新周期。进近串→CIFP ident 逐字匹配（唯一别名 `RNP`↔`RNAV`）。纯标准库，失败→`None` |
...
- **F26 羽田实测运用状况(✅ v2.0.0_alpha1)**:用户发现国土交通省的「羽田空港飛行コースホームページ」(`ntrack.mlit.go.jp`)**全公开**羽田的实测运用状况——当前实际在用的**进近方式 + 落地跑道 + 离场跑道**,每 30 分钟一条、覆盖过去 72 小时。**这正是 F24 规则引擎费劲去【推断】的东西,而它是权威实测值。**
  - **取法(已实测)**:一次 GET `https://www.ntrack.mlit.go.jp/NtrackTop/show`,110KB HTML,表格**内联**在首页(`<table id='atistable'>`),不走 ajax、不需 JS、不需认证——与 jp-routes / FlightAware 同族;站点无 robots 限制。串格式极规整:`<进近> LDG RWY <落地> DEP RWY <离场>`。
  - **意外之喜**:F23 为 CIFP 合成的进近显示名与 MLIT 的写法**逐字一致**(两边都循 ICAO 图表命名)——`LDA W RWY22`↔`X22-W`、`ILS Y RWY34R`↔`I34RY`。**唯一别名是 `RNP`↔`RNAV`**。
  - **分工(用户定)**:**ntrack 定「用哪些跑道 + 什么进近」(硬约束),`operation.json` 规则定「在其中怎么选 + 配什么 SID/STAR」**(SID/STAR 正是 ntrack 给不了的)。多条跑道在用时,比对规则找最吻合的一条,据此默认算一个。
  - **关键教训(实现中被冒烟打回来才想清的)**:光把跑道候选收窄给规则引擎**不够**。风闸与天气闸**本就是我们用来【推断】构型的代理指标**;实测既然直接给了构型,这两道闸就该**由它取代**,而不是继续拦路——否则会出现「顺风 9.2 节、差 0.8 节没够到 10 节换向门槛,于是把南風规则全闸掉,而 MLIT 明明正在跑都心運用」的荒谬情形(**实测复现过**)。故 `select_rule(config=)` 时:跑道对不上实测的规则排除、`skip_wind`/`skip_weather`、**不再做顺/侧风超限剔除**(实测在用的跑道就是现实)、**实测进近成为好天/恶天的判别依据**(`LDA W RWY22`⇒好天、`ILS RWY22`⇒恶天,比猜云底直接得多);**时段/星期闸门保留**(ntrack 不说 A/B/C 哪一档,那得靠 EOBT)。
  - **为何只取最新一条**(用户定):模拟飞行常调时间但**用实时天气**,而运用构型正是被实时天气驱动的——「此刻的构型」就是用户实际会遇到的那个。按钟点切换的深夜運用由规则引擎的时段闸门兜住。
  - **边界**:仅羽田(成田是 NAA 另一站点,可后续接入);断网/改版/解析失败 → `None`,静默回退规则引擎,规划流程不受影响。跑道下拉**仍列全部跑道**(实测在用的标 ✅ 并排最前),用户永远能改选——决策支持,不替用户拍板。
...
- **v2.0.0 → v2.0.0_alpha1(✅ 预览版)**:
  - **羽田实测运用状况(F26)**:接入国土交通省 ntrack,把羽田的**实测**进近/落地/离场构型作为进离场预选的**首选依据**,规则引擎(F24)退为其余机场与回退路径。不升主版本——功能仍属 v2.0.0 的范畴,发 alpha 让它先经受实机检验。
